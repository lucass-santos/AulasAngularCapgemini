import { Component, OnInit } from '@angular/core';
import { Celular } from '../types/Celular';

@Component({
  selector: 'app-renerizando-listas',
  templateUrl: './renerizando-listas.component.html',
  styleUrls: ['./renerizando-listas.component.css']
})
export class RenerizandoListasComponent {

 celulares: Celular[] = [
  {id: 1, nome: "Celular XL", descricao: "Um celular grande", esgotado: false},
  {id: 2, nome: "Celular Normal", esgotado: false},
  {id: 3, nome: "Celular Mini", descricao: "Um celular pequeno", esgotado: true}
]

}
